...
class SRNet(nn.Module):
    def __init__(self, args):
        ...
        multi_scale = args.multi_scale # value of multi_scale is multi_scale in args.
        group = args.group 
        kernel_size = 3 
        padding = 1     
        features = 64    
        channels = args.n_channels
        self.sub_mean = common.MeanShift((0.4488, 0.4371, 0.4040), sub=True)
        self.add_mean = common.MeanShift((0.4488, 0.4371, 0.4040), sub=False)
        '''
        ...
        self.upsample = common.UpsampleBlock(64, scale=self.scale, multi_scale=multi_scale,group=1)
    def forward(self, x):
        x = self.sub_mean(x)
        x1 = self.conv1_1(x)
        b1 = self.b1(x1)
        ...
        x2 = self.conv2(b6)
        temp = self.upsample(x2, scale=self.scale)
